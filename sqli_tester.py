...
def main():
    ...
    try:
        with open('urls_and_forms.json', 'r') as f:
            data = json.load(f)
    except FileNotFoundError:
        print("[-] urls_and_forms.json not found. Run the crawler first.")
        sys.exit(1)

    all_vulnerabilities = []

    for url, forms in data.items():
        if 'vulnerabilities' in url and forms:
            print(f"\n[*] Testing: {url}")
            for form in forms:
                print(f"[*] Generating LLM payloads for form at {url}...")
